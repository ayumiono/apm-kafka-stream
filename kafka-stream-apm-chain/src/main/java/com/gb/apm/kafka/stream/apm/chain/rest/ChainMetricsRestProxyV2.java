...
import com.gb.apm.kafka.stream.apm.common.model.ChainTopMetrics;
...
public class ChainMetricsRestProxyV2 {
	@Autowired
	...
	public List<ChainMetricsV2> metrics(@PathVariable("chain_id") long chain_id) {
		try {
			...
			WindowStoreIterator<ChainMetricsV2> methodMetrics = store.fetch(chain_id, queryWindow.start(), queryWindow.end());
			
			List<ChainMetricsV2> result = new ArrayList<>();
	...
	@RequestMapping(value="/timebucket/{chain_id}",method=RequestMethod.GET)
	public List<TimeBucket> getTimeBucket(@PathVariable("chain_id") long chain_id) {
		try {
			ReadOnlyWindowStore<Long, ChainMetricsV2> store = 
					app.streams.store(KafkaStreamApp.CHAIN_METRICS_STORE, QueryableStoreTypes.windowStore());
	...
	@RequestMapping(value="/top/{systemId}",method=RequestMethod.GET)
	public List<ChainTopMetrics> top(@RequestParam("top") int top,@PathParam("systemId") Integer systemId) {
		ReadOnlyKeyValueStore<Integer, PriorityQueue<ChainTopMetrics>> store = app.streams.store(KafkaStreamApp.TOP_SPENT_CHAIN, QueryableStoreTypes.keyValueStore());
		PriorityQueue<ChainTopMetrics> queue = store.get(systemId);
		List<ChainTopMetrics> result = new ArrayList<>();
		for(int i=0;i<top;i++) {
			ChainTopMetrics item = queue.peek();
			if(item != null) {
				result.add(item);
			}
		}
