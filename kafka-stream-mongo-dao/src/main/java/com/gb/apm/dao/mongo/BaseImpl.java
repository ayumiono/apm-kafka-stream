...
public class BaseImpl<TDocument> {
	...
	protected BaseImpl(Class<TDocument> clazz) throws Exception {
		this.clazz = clazz;
	...
	protected BaseImpl(Class<TDocument> clazz,String collection,String idField) throws Exception {
		this.clazz = clazz;
		if(collection == null) {
			throw new GBMongodbException("collection cannot be null");
		}
	...
	public String insert(TDocument document) throws GBMongodbException {
		...
		try {
			DBCursor<TDocument> cursor = _collection.get().find(ormIdQuery(document));
			if(cursor.hasNext()) {
				this.updateById(document);
			} else {
				WriteResult<TDocument, String> ir = _collection.get().insert(document);
//				return ir.getSavedId();//java.lang.ClassCastException: org.bson.types.ObjectId cannot be cast to java.lang.String
			}
			return null;
		} catch (Exception e) {
	...
	public PageResult<TDocument> page(PageResult<TDocument> page,TDocument condition) throws GBMongodbException{
		if(_collection.get() == null) {
			throw new GBMongodbException("please chose the right mongo database first");
		}
		if(page.getPageSize() == null || page.getPageSize() <= 0) page.setPageSize(10);
		if(page.getPageSize() == null) page.setPageNo(1);
		try {
			List<TDocument> result = new ArrayList<>();
			DBCursor<TDocument> rs = _collection.get().find(ormQuery(condition)).limit(page.getPageSize()).skip((page.getPageNo()-1)*page.getPageSize());
			rs.forEach(new Consumer<TDocument>() {
				@Override
				public void accept(TDocument t) {
					result.add(t);
			...
			int totalCount = _collection.get().find(ormQuery(condition)).count();
			page.setRs(result);
			page.setTotalCount(totalCount);
			return page;
		} catch (Exception e) {
			throw new GBMongodbException(String.format("%s page query exception:Page【%s】 %s", this.collectionName,page,condition), e);
		}
	}
...
//					logger.error("ormQuery invoke error",e);
	...
	public static void main(String[] args) throws Exception {
		BaseImpl<Chain> b = new BaseImpl<>(Chain.class);
		System.out.println(b.idField);
	}
}
